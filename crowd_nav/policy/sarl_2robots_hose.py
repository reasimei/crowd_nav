...
class LLMEnhancedValueNetwork(HierarchicalValueNetwork):
    def __init__(self, input_dim, self_state_dim, mlp1_dims, mlp2_dims, mlp3_dims, attention_dims, 
                 with_global_state, cell_size, cell_num):
        super().__init__(input_dim, self_state_dim, mlp1_dims, mlp2_dims, mlp3_dims, attention_dims, 
                        with_global_state, cell_size, cell_num)
        
        # LLM建议融合层
        self.llm_fusion = nn.Sequential(
            nn.Linear(mlp3_dims[-1] + 3, mlp3_dims[-1]),  # +3 for llm risk and recommendation
            nn.ReLU(),
            nn.Linear(mlp3_dims[-1], mlp3_dims[-1]),
            nn.ReLU()
        )
        
    def forward(self, state, llm_input=None, training_phase=None):
        # 获取基础网络的输出
        base_value = super().forward(state, training_phase)
        ...
        if llm_input is not None:
            try:
                ...
                # 将基础值与LLM特征连接并通过融合层
                combined = torch.cat([base_value, llm_features.unsqueeze(0)], dim=1)
                enhanced_value = self.llm_fusion(combined)
                
                return enhanced_value
                
            except Exception as e:
                logging.warning(f"Error in LLM fusion: {e}")
                return base_value
...
class HierarchicalSARL(SARL):
    ...
    def predict(self, state):
        ...
        if max_action is None:
            ...
            logging.warning('Using default action due to prediction failure')
        
        self.last_value = max_value if max_value != float('-inf') else 0
        return max_action
...
class HierarchicalLLMSARL(HierarchicalSARL):
    ...
    def configure(self, config):
        ...
        # 初始化LLM配置
        self.llm_enabled = config.getboolean('llm', 'enabled', fallback=False)
        api_key = config.get('llm', 'api_key', fallback=os.getenv('DEEPSEEK_API_KEY'))
        self.use_llm_in_training = config.getboolean('llm', 'use_in_training', fallback=False)
        
        # 从配置中获取速度参数
        self.v_pref = config.getfloat('robot', 'v_pref', fallback=1.0)

        if self.llm_enabled and api_key:
            self.llm_decision_maker = LLMDecisionMaker(api_key)
