...
class MultiHumanRL(CADRL):
    ...
    def transform(self, state):
        ...
        :param state:
        :return: tensor of shape (# of humans + # of other robots, combined state length)
        """
        state_tensors = []
        
        # Process human states
        for human_state in state.human_states:
            human_tensor = torch.Tensor([state.self_state + human_state]).to(self.device)
            state_tensors.append(human_tensor)
        
        # Process other robot states
        for robot_state in state.other_robot_states:
            # Create a "robot" tensor similar to human tensor
            robot_tensor = torch.Tensor([state.self_state + robot_state]).to(self.device)
            state_tensors.append(robot_tensor)
        
        if not state_tensors:
            return None  # No humans or other robots to observe
