...
class MultiHumanRL(CADRL):
    ...
    def predict(self, state):
        ...
        # During training phase, use epsilon-greedy exploration
        if self.phase == 'train' and probability < self.epsilon:
            ...
            # Initialize default action in case no better action is found
            ...
            try:
                for action in self.action_space:
                    ...
                    # Safe handling of state batching
                    try:
                    ...
                    # VALUE UPDATE with error handling
                    try:
    ...
    def compute_reward(self, nav, humans, phase='human_avoidance'):
        if phase == 'human_avoidance':
            return self.compute_human_avoidance_reward(nav, humans)
        else:
            return self.compute_robot_coordination_reward(nav, humans)
    ...
    def compute_robot_coordination_reward(self, nav, humans):
        # 新的机器人协调阶段奖励计算
        reward = 0
        
        # 1. 基础奖励(到达目标)
        dist_to_goal = np.linalg.norm((nav.px - nav.gx, nav.py - nav.gy))
        reward += -dist_to_goal * 0.1
        
        # 2. 软管约束奖励
        if hasattr(nav, 'pair_robot'):
            dist_to_pair = np.linalg.norm((nav.px - nav.pair_robot.px, 
                                        nav.py - nav.pair_robot.py))
            hose_diff = abs(dist_to_pair - self.hose_length)
            reward -= hose_diff * 0.5
        
        # 3. 避免与静态human碰撞
        for human in humans:
            if human.reached_destination():
                dist = np.linalg.norm((nav.px - human.px, nav.py - human.py))
                if dist < self.discomfort_dist:
                    reward -= (self.discomfort_dist - dist) * 0.5
