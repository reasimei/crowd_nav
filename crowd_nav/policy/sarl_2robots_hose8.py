...
class HierarchicalSARL(SARL):
    ...
    def predict(self, state):
        ...
        if (self.phase == 'train' and np.random.random() < 0.15):  # 15% 随机探索率
            use_random_action = True
            random_action = self.action_space[np.random.randint(len(self.action_space))]
            logging.debug("Using random exploration action")
        
        if not use_random_action:
            # 正常动作评估
            max_value = float('-inf')
            max_action = None
                ...
                try:
                    ...
                    if all_agent_states:  # 确保有其他智能体
                        ...
                        if hasattr(self.model, 'training_phase'):
                            current_phase = self.training_phase if hasattr(self, 'training_phase') else 'human_avoidance'
            ...
            max_value = 0.0  # 随机探索时不计算值
        
        self.last_value = getattr(self, 'last_value', 0) if not hasattr(locals(), 'max_value') else max_value
        return max_action
    ...
    def compute_reward(self, next_state, next_human_states, next_robot_states=None):
        ...
        重新设计的奖励函数，确保明确的奖励信号
        """
        # 主奖励：使用环境状态（与原始代码保持一致）
        if hasattr(self, 'env') and self.env is not None:
            if hasattr(self.env, 'success') and self.env.success:
                return 10.0  # 保持原始高成功奖励
            elif hasattr(self.env, 'collision') and self.env.collision:
                return -2.5  # 保持原始高碰撞惩罚
        
        # 次级奖励：直接状态检查（无法访问环境状态时的后备）
        reward = 0
        ...
        # 接近惩罚（鼓励保持距离）
        if min_human_dist < 1.0:
            reward -= 0.5 * (1.0 - min_human_dist)  # 距离越近，惩罚越大
        
        if next_robot_states and min_robot_dist < 2.0:
            reward -= 0.7 * (2.0 - min_robot_dist)  # 机器人之间保持更大距离
        
        # 朝向目标的奖励（使用方向点积，优于距离比较）
        ...
        if np.linalg.norm(goal_dir) > 0:
            ...
            if np.linalg.norm(current_dir) > 0:
                ...
                reward += 0.4 * np.dot(goal_dir, current_dir)  # 增强朝向目标的奖励
        
        # 进步奖励（使用距离变化）
        if hasattr(self, 'last_dist_to_goal'):
            progress = self.last_dist_to_goal - dist_to_goal
            reward += progress * 0.3  # 增强进步奖励，鼓励机器人朝目标移动
        self.last_dist_to_goal = dist_to_goal
        
        # 训练阶段特定调整（区分人类避障和机器人协调）
        if hasattr(self, 'training_phase'):
            if self.training_phase == 'human_avoidance':
                if min_human_dist > 1.0:  # 远离人类时给予更多奖励
                    reward += 0.3
            elif self.training_phase == 'robot_avoidance':
                if min_robot_dist > 2.0:  # 远离其他机器人时给予更多奖励
                    reward += 0.3
