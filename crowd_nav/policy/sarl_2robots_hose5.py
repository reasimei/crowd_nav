...
class HierarchicalSARL(SARL):
    ...
    def initialize_action_space(self):
        ...
        self.action_space = []
        holonomic = True  # 根据配置设置
        
        # 设置速度和旋转选项
        speeds = [(np.exp((i + 1) / 5) - 1) / (np.e - 1) * self.v_pref for i in range(5)]
        if holonomic:
        ...
        for rotation, speed in itertools.product(rotations, speeds):
            if holonomic:
                self.action_space.append(ActionXY(speed * np.cos(rotation), speed * np.sin(rotation)))
            else:
                self.action_space.append(ActionRot(rotation, speed))
