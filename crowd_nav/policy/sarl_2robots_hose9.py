    ...
    def compute_reward(self, next_state, next_human_states, next_robot_states=None):
        ...
        if hasattr(self, 'env') and self.env is not None:
            if hasattr(self.env, 'all_humans_at_goal'):
                ...
                # 检查人类是否稳定在目标位置
                if hasattr(self.env, 'humans_stable_at_goal'):
                    humans_stable_at_goal = self.env.humans_stable_at_goal
        ...
        if next_robot_states:
            for robot in next_robot_states:
                if robot.px != next_state.px or robot.py != next_state.py:  # 不是同一机器人
                    # 跳过软管伙伴
                    if has_hose_partner and hose_partner is not None and robot.robot_index == hose_partner.robot_index:
                        continue
        ...
        # 软管与其他软管的碰撞检查
        other_hose_collision = False
        min_hose_to_hose_dist = float('inf')
        ...
        if use_hose and has_hose_partner and hose_partner is not None:
            try:
                ...
                if hose_length > 0:
                    ...
                    # 检查软管与其他软管之间的碰撞
                    if next_robot_states and len(next_robot_states) >= 2:
                        # 获取当前机器人的软管对编号
                        curr_pair_idx = next_state.robot_index // 2
                        
                        # 遍历所有可能的机器人对
                        for i in range(0, len(self.env.robots), 2):
                            if i + 1 < len(self.env.robots):
                                pair_idx = i // 2
                                # 跳过自己的软管对
                                if pair_idx == curr_pair_idx:
                                    continue
                                ...
                                # 计算两个软管之间的最短距离
                                # 在每个软管上取多个采样点
                                for t1 in np.linspace(0, 1, 5):  # 自己软管上的5个采样点
                                    # 自己软管上的点
                                    p1 = np.array([
                                        next_state.px * (1-t1) + hose_partner.px * t1,
                                        next_state.py * (1-t1) + hose_partner.py * t1
                                    ])
                                    
                                    # 在另一对软管上采样
                                    for t2 in np.linspace(0, 1, 5):  # 对方软管上的5个采样点
                                        # 对方软管上的点
                                        p2 = np.array([
                                            other_robot1.px * (1-t2) + other_robot2.px * t2,
                                            other_robot1.py * (1-t2) + other_robot2.py * t2
                                        ])
                                        
                                        # 计算两点之间的距离
                                        dist = np.linalg.norm(p1 - p2) - 2 * hose_thickness
                                        
                                        # 更新最小距离
                                        if dist < min_hose_to_hose_dist:
                                            min_hose_to_hose_dist = dist
        ...
        if human_collision:
            ...
            reward -= 3.5  # 软管与软管碰撞惩罚最严重
            logging.debug(f"惩罚：软管与软管碰撞 -3.5")
        else:
            # 安全距离奖励/惩罚
            ...
            # 软管与软管安全距离惩罚
            if use_hose and min_hose_to_hose_dist < 1.0:
                # 人类稳定时强化对软管-软管碰撞的避免（因为此时应该专注于避开其他软管）
                hose_to_hose_penalty_factor = 2.0 if humans_stable_at_goal else 1.5
                penalty = (1.0 - min_hose_to_hose_dist) * hose_to_hose_penalty_factor
                reward -= penalty
                logging.debug(f"惩罚：软管接近其他软管 -{penalty:.2f}")
            ...
            # 软管伙伴协调奖励 - 当人类稳定时
            if humans_stable_at_goal and has_hose_partner and hose_partner is not None:
                # 如果两个机器人都离目标较远，需要更好的协调
                robot_goal_dist = dist_to_goal
                partner_goal_dist = np.linalg.norm([
                    hose_partner.gx - hose_partner.px,
                    hose_partner.gy - hose_partner.py
                ])
                
                if robot_goal_dist > 1.0 or partner_goal_dist > 1.0:
                    # 计算自己的朝向
                    if robot_goal_dist > 0.1:  # 避免除以0
                        robot_goal_dir = goal_dir / robot_goal_dist
                        ...
                        hose_dist = np.linalg.norm(hose_vec)
                        
                        if hose_dist > 0.1:  # 避免除以0
                            hose_dir = hose_vec / hose_dist
                            
                            # 计算方向一致性
                            alignment = np.dot(robot_goal_dir, hose_dir)
                            
                            # 当方向一致性较高时，给予协调奖励
                            if alignment > 0.7:  # 方向基本一致
                                coordination_reward = 0.2
                                reward += coordination_reward
                                logging.debug(f"奖励：软管伙伴方向协调 +{coordination_reward:.2f}")
        
        logging.debug(f"Total reward: {reward:.4f}")
        return reward
