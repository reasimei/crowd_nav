...
class Explorer(object):
    ...
    def run_k_episodes(self, k, phase, update_memory=False, imitation_learning=False, episode=None,
            ...
            while not done:
                ...
                for robot in self.robots:
                    ...
                    if self.training_phase == 'human_avoidance':
                        ...
                        action.append(robot.act_avoid_humans(ob))
                    elif self.training_phase == 'robot_avoidance':
                        # Focus on avoiding other robots
                        action.append(robot.act_avoid_robots(ob))
                    else:
                        # Normal action selection
                        action.append(robot.act(ob))
        ...
        # Switch training phase based on performance
        if phase == 'train' and episode is not None:
            if self.training_phase == 'human_avoidance' and success_rate > 0.9:
                self.training_phase = 'robot_avoidance'
                logging.info('Switching to robot avoidance training phase')
