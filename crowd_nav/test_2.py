    ...
    if args.visualize:
        ...
        for episode in range(env.case_size[args.phase]):
            ...
            while not done:
                ...
                # Wait until pedestrian positions are received from all robots
                while len(pedestrian_positions) < 2:
                    rospy.sleep(0.01)  # Wait for callbacks

                # Combine pedestrian positions from all robots
                all_pedestrian_positions = []
                for positions in pedestrian_positions.values():
                    all_pedestrian_positions.extend(positions)

                # Clear pedestrian_positions for next iteration
                pedestrian_positions.clear()

                # Update the environment's human positions
                # Assuming env.humans is a list of Human agents with set_position method
                for i, human in enumerate(env.humans):
                    if i < len(all_pedestrian_positions):
                        pos = all_pedestrian_positions[i]
                        human.set_position((pos.x, pos.y))
                    else:
                        # Hide or remove extra humans if fewer pedestrians detected
                        human.set_position((float('inf'), float('inf')))
...
if __name__ == '__main__':
    ...
    # Initialize pedestrian positions dictionary
    pedestrian_positions = {}

    def pedestrian_callback(msg, robot_index):
        pedestrian_positions[robot_index] = msg.positions

    # Subscribe to pedestrian positions from each robot
    for robot_index in [1, 2]:
        rospy.Subscriber(f'/robot_{robot_index}/pedestrian_positions', PedestrianPositions,
                        pedestrian_callback, robot_index)
