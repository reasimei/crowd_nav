...
def main():
    ...
    package_path = rospack.get_path('crowd_nav')  # Use your package name

    # Use absolute paths for the config files
    env_config_file = os.path.join(package_path, 'configs', 'env.config')
    policy_config_file = os.path.join(package_path, 'configs', 'policy.config')

    # Log the config file paths
    logging.info('Environment config file: %s', env_config_file)
    logging.info('Policy config file: %s', policy_config_file)

    # Check if config files exist
    if not os.path.exists(env_config_file):
        logging.error('Environment config file does not exist: %s', env_config_file)
        return

    if not os.path.exists(policy_config_file):
        logging.error('Policy config file does not exist: %s', policy_config_file)
        return
    ...
    # Add logging to confirm config file paths
    logging.info('Environment config file: %s', env_config_file)
    logging.info('Policy config file: %s', policy_config_file)
    ...
    def position_callback(msg, robot_index):
        robot_positions[robot_index] = [msg.pose.position.x, msg.pose.position.y]

    # Subscribe to robot positions and create publishers for velocities
    for i in range(len(robot)):
        rospy.Subscriber(f'/robot_{i}/position', PoseStamped, lambda msg, idx=i: position_callback(msg, idx))
        velocity_pubs.append(rospy.Publisher(f'/robot_{i}/cmd_vel', Twist, queue_size=10))

    rate = rospy.Rate(10)  # 10 Hz

    while not rospy.is_shutdown():
        if len(robot_positions) == len(robot):
            # Create observation using real-time positions
            ob = env.get_observation(robot_positions)
            actions = []
            for r in robot:
                action = r.policy.predict(ob)
                actions.append(action)

            # Publish planned velocities to robots
            for i, action in enumerate(actions):
                twist_msg = Twist()
                twist_msg.linear.x = action.vx
                twist_msg.linear.y = action.vy
                velocity_pubs[i].publish(twist_msg)
        rate.sleep()
    ...
    if args.visualize:
        ...
        # Save the animation
        if args.traj:
            anim.save('./result/test_009.gif', writer='pillow', fps=10)
