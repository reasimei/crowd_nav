...
class CrowdSim(gym.Env):
    ...
    def step(self, action1, action2, update=True):
        ...
        if distance_between_robots > self.hose_length * 2:
            reward += self.hose_penalty * (distance_between_robots - self.hose_length * 2)
            collision = True
        ...
        if mode == 'human':
            ...
            for k in range(total_time_steps):
                ...
                if k % interval == 0 or k == total_time_steps - 1:
                    ...
                    interval_steps = int(2 / self.time_step)

                    for step in range(0, total_time_steps, interval_steps):
                        # Plot hose
                        robot1_pos = np.array(robot_positions[0][step])
                        robot2_pos = np.array(robot_positions[1][step])
            ...
            if not os.path.exists('./result'):
                os.makedirs('./result')
            plt.savefig("./result/test_002.png", dpi=300)
            #plt.show()
