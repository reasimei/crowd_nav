...
class CrowdSim(gym.Env):
    ...
    def step1(self, actions, update=True):
        """
        Modified step1 function with proper reward accumulation
        """
        # Initialize reward
        reward = 0
        done = False
        ...
        for i in range(0, self.robot_num, 2):
            if i + 1 < self.robot_num:
                ...
                if distance > self.hose_length:
                    hose_penalty = self.hose_penalty * (distance - self.hose_length)
                    hose_reward += hose_penalty
                    #print(f"Debug - Hose violation: distance={distance}, penalty={hose_penalty}")

        # 累积软管惩罚到总 reward
        reward += hose_reward
        #print(f"Debug - After hose reward: {reward}")
        ...
        #print(f"Debug - Collision: {collision}, Current reward: {reward}")
        ...
        if collision:
            ...
            info = Collision(human_collision, robot_collision)
            
            if info.get_human_collision():
                collision_reward *= 2.0
            if info.get_robot_collision():
                collision_reward *= 1.5
            
            # 碰撞发生时，直接设置 reward 为碰撞惩罚
            reward = collision_reward
            done = True
            #print(f"Debug - Collision reward: {collision_reward}")
        elif dmin < self.discomfort_dist:
            # 不适惩罚应该累积到现有的 reward 上
            discomfort_reward = (dmin - self.discomfort_dist) * self.discomfort_penalty_factor * self.time_step
            reward += discomfort_reward
            #print(f"Debug - Added discomfort reward: {discomfort_reward}")
        ...
        # 如果所有机器人都到达目标
        if all(robot.reached_destination() for robot in self.robots):
