...
class CrowdSim(gym.Env):
    ...
    def configure(self, config):
        ...
        self.hose_length = self.config.getfloat('hose', 'length')
        ...
        self.hose_mass = config.getfloat('hose', 'mass')
    ...
    def reset(self, phase='test', test_case=None):
        ...
        # Initialize hose with proper connection and smooth curve
        robot1_pos = np.array(self.robot1.get_position())
        robot2_pos = np.array(self.robot2.get_position())
        
        # Calculate initial segment positions using cubic spline interpolation
        segment_num = self.hose_segments + 1  # Include endpoints
        self.hose_points = []
        self.hose_velocities = []
        
        # Calculate total straight-line distance between robots
        total_distance = np.linalg.norm(robot2_pos - robot1_pos)
        
        # Create smooth curve between robots ensuring total length is hose_length
        t = np.linspace(0, 1, segment_num)
        # Add small vertical offset to create natural curve
        mid_point = (robot1_pos + robot2_pos) / 2
        control_point = mid_point + np.array([0, 0.3 * self.hose_length])
        
        # Quadratic Bezier curve for smooth initialization
        for ti in t:
            # Quadratic Bezier curve formula
            pos = (1-ti)**2 * robot1_pos + \
                  2*(1-ti)*ti * control_point + \
                  ti**2 * robot2_pos
            self.hose_points.append(pos)
            self.hose_velocities.append(np.zeros(2))
        
        # Adjust segment positions to maintain exact hose length
        self._enforce_length_constraint()
        
        return ob
    

    def _enforce_length_constraint(self):
        """Adjust hose segments to maintain proper length while preserving smooth curve"""
        segment_length = self.hose_length / self.hose_segments
        
        # Multiple iterations to converge to desired length
        for _ in range(10):
            # Forward pass
            for i in range(1, len(self.hose_points)):
                current = np.array(self.hose_points[i])
                previous = np.array(self.hose_points[i-1])
                delta = current - previous
                dist = np.linalg.norm(delta)
                if dist > segment_length:
                    # Move current point closer while maintaining direction
                    correction = (dist - segment_length) / dist * delta
                    self.hose_points[i] = previous + delta - correction * 0.5
                
            # Backward pass
            for i in range(len(self.hose_points)-2, -1, -1):
                current = np.array(self.hose_points[i])
                next_point = np.array(self.hose_points[i+1])
                delta = current - next_point
                dist = np.linalg.norm(delta)
                if dist > segment_length:
                    # Move current point closer while maintaining direction
                    correction = (dist - segment_length) / dist * delta
                    self.hose_points[i] = next_point + delta - correction * 0.5

    def update_hose_dynamics(self):
        """Update hose physics with improved smoothness constraints"""
        EPSILON = 1e-8
        dt = self.time_step
        segment_length = self.hose_length / self.hose_segments
        
        # First, update endpoints to match robot positions
        self.hose_points[0] = np.array(self.robot1.get_position())
        self.hose_points[-1] = np.array(self.robot2.get_position())
        
        # Update internal points with smoothness constraint
        for i in range(1, len(self.hose_points) - 1):
            # Current point state
            pos = np.array(self.hose_points[i])
            vel = np.array(self.hose_velocities[i])
            
            # Calculate forces including smoothness constraint
            total_force = np.zeros(2)
            
            # Spring forces from neighbors
            for neighbor_idx in [i-1, i+1]:
                neighbor_pos = np.array(self.hose_points[neighbor_idx])
                delta = neighbor_pos - pos
                distance = np.linalg.norm(delta)
                
                if distance > EPSILON:
                    direction = delta / distance
                    stretch = distance - segment_length
                    spring_force = self.hose_stiffness * stretch * direction
                    rel_vel = self.hose_velocities[neighbor_idx] - vel
                    damping_force = self.hose_damping * np.dot(rel_vel, direction) * direction
                    total_force += spring_force + damping_force
            
            # Add smoothness constraint force
            if i > 0 and i < len(self.hose_points) - 1:
                prev_pos = np.array(self.hose_points[i-1])
                next_pos = np.array(self.hose_points[i+1])
                expected_pos = (prev_pos + next_pos) / 2
                smoothness_force = self.hose_stiffness * 0.5 * (expected_pos - pos)
                total_force += smoothness_force
            
            # Apply force limits and update position
            force_magnitude = np.linalg.norm(total_force)
            max_force = 50.0
            if force_magnitude > max_force:
                total_force *= max_force / force_magnitude
            ...
            if vel_magnitude > max_velocity:
                new_vel *= max_velocity / vel_magnitude
            
            # Update position
            new_pos = pos + new_vel * dt
            
            # Store updates
            self.hose_velocities[i] = new_vel
            self.hose_points[i] = new_pos
        
        # Enforce length constraints after dynamics update
        self._enforce_length_constraint()



    def calculate_hose_forces(self):
        """Calculate forces on robots from hose with proper constraints"""
        EPSILON = 1e-8
        segment_length = self.hose_length / self.hose_segments
        
        # Force on robot1 (from first segment)
        force_on_robot1 = np.zeros(2)
        delta1 = self.hose_points[1] - self.hose_points[0]
        dist1 = np.linalg.norm(delta1)
        
        if dist1 > EPSILON:
            direction1 = delta1 / dist1
            stretch1 = dist1 - segment_length
            v_rel1 = self.hose_velocities[1] - np.array([self.robot1.vx, self.robot1.vy])
            
            # Calculate force with proper scaling
            spring_force1 = self.hose_stiffness * stretch1 * direction1
            damping_force1 = self.hose_damping * np.dot(v_rel1, direction1) * direction1
            force_on_robot1 = spring_force1 + damping_force1
        
        # Force on robot2 (from last segment)
        force_on_robot2 = np.zeros(2)
        delta2 = self.hose_points[-1] - self.hose_points[-2]
        dist2 = np.linalg.norm(delta2)
        
        if dist2 > EPSILON:
            direction2 = delta2 / dist2
            stretch2 = dist2 - segment_length
            v_rel2 = self.hose_velocities[-2] - np.array([self.robot2.vx, self.robot2.vy])
            
            # Calculate force with proper scaling
            spring_force2 = self.hose_stiffness * stretch2 * direction2
            damping_force2 = self.hose_damping * np.dot(v_rel2, direction2) * direction2
            force_on_robot2 = -(spring_force2 + damping_force2)  # Note the negative sign
        
        # Limit maximum force
        max_force = 50.0  # Newtons
        for force in [force_on_robot1, force_on_robot2]:
    ...
    def step(self, action1, action2, update=True):
        ...
        # Update robots' velocities based on actions and hose forces
        if self.robot1.kinematics == 'holonomic' and self.robot2.kinematics == 'holonomic':
            # Get hose forces
            force_on_robot1, force_on_robot2 = self.calculate_hose_forces()
            
            # Apply forces to robots through their apply_force method
            self.robot1.apply_force(force_on_robot1)
            self.robot2.apply_force(force_on_robot2)
            
            # Combine action velocities with force-induced velocities
            # Robot 1
            total_vx1 = action1.vx + self.robot1.vx
            total_vy1 = action1.vy + self.robot1.vy
            speed1 = np.hypot(total_vx1, total_vy1)
            if speed1 > self.robot1.v_pref:
                total_vx1 *= self.robot1.v_pref / speed1
                total_vy1 *= self.robot1.v_pref / speed1
            self.robot1.vx = total_vx1
            self.robot1.vy = total_vy1
            
            # Robot 2
            total_vx2 = action2.vx + self.robot2.vx
            total_vy2 = action2.vy + self.robot2.vy
            speed2 = np.hypot(total_vx2, total_vy2)
            if speed2 > self.robot2.v_pref:
                total_vx2 *= self.robot2.v_pref / speed2
                total_vy2 *= self.robot2.v_pref / speed2
            self.robot2.vx = total_vx2
            self.robot2.vy = total_vy2

        # Update hose dynamics
    ...
    def apply_hose_forces(self):
        """Apply hose forces with speed constraints"""
        force_on_robot1, force_on_robot2 = self.calculate_hose_forces()
        
        # Apply forces through robots' apply_force method
        self.robot1.apply_force(force_on_robot1)
        self.robot2.apply_force(force_on_robot2)
        
        # Ensure speeds remain within limits after force application
        for robot in [self.robot1, self.robot2]:
            speed = np.linalg.norm([robot.vx, robot.vy])
            if speed > robot.v_pref:
                robot.vx = robot.vx / speed * robot.v_pref
                robot.vy = robot.vy / speed * robot.v_pref
