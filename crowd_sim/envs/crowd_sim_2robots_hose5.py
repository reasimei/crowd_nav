    ...
    def step1(self, actions, update=True):
            ...
            for j in range(self.robot_num):
                for k in range(j + 1, self.robot_num):
                    ...
                    if self.robots[j].kinematics == 'holonomic' and self.robots[k].kinematics == 'holonomic':
                        ...
                        vx = (actions[j].v * np.cos(actions[j].r + self.robots[j].theta) - 
                            actions[k].v * np.cos(actions[k].r + self.robots[k].theta))
                        vy = (actions[j].v * np.sin(actions[j].r + self.robots[j].theta) - 
                            actions[k].v * np.sin(actions[k].r + self.robots[k].theta))
        ...
        for i in range(len(distance_between_robots_pair)):
            if distance_between_robots_pair[i] > self.hose_length * 1.5:
                reward += self.hose_penalty * (distance_between_robots_pair[i] - self.hose_length * 1.5)
                #collision = True
