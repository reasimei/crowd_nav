...
class CrowdSim(gym.Env):
    ...
    def render(self, mode='traj', output_file='/home/zzy/catkin_ws/src/crowd_nav/crowd_nav/result'):
        ...
        if mode == 'human':
            ...
            # Define base colors for robots - using more distinct colors
            robot_base_color_names = ['navy', 'teal', 'forestgreen', 'goldenrod', 
                                     'dimgray', 'darkviolet', 'maroon', 'darkolivegreen']
            ...
            robot_light_colors = []
            robot_dark_colors = []
            robot_medium_colors = []  # 中间颜色用于图例
            for i in range(self.robot_num):
                color_name = robot_base_color_names[i % len(robot_base_color_names)]
                light_color = adjust_color(color_name, amount=0.7, lighten=True)  # 更亮的初始颜色
                dark_color = adjust_color(color_name, amount=0.3, lighten=False)  # 更暗的结束颜色
                # 计算中间颜色（用于图例）
                medium_color = interpolate_color(light_color, dark_color, 0.5)
                
                robot_light_colors.append(light_color)
                robot_dark_colors.append(dark_color)
                robot_medium_colors.append(medium_color)

            # Create light and dark colors for humans
            human_light_colors = []
            human_dark_colors = []
            human_medium_colors = []  # 中间颜色用于图例
            for i in range(len(self.humans)):
                ...
                light_color = adjust_color(base_color_rgb, amount=0.7, lighten=True)
                dark_color = adjust_color(base_color_rgb, amount=0.3, lighten=False)
                # 计算中间颜色（用于图例）
                medium_color = interpolate_color(light_color, dark_color, 0.5)
                
                human_light_colors.append(light_color)
                human_dark_colors.append(dark_color)
                human_medium_colors.append(medium_color)
            ...
            for k in range(total_time_steps):
                ...
                if k % interval == 0 or k == total_time_steps - 1:
                    # Plot humans first (圆形, 在下层)
                    humans = []
                    for i in range(len(self.humans)):
                        # 使用Circle并添加透明度
                        human_circle = plt.Circle(human_positions[k][i], self.humans[i].radius*0.8,
                                            facecolor='white',  # 空心圈
                                            edgecolor= human_colors[i], linewidth=8.0, zorder=2)
                    ...
                    for i in range(self.robot_num):
                        ...
                        robot_circle = plt.Circle(robot_positions[i][k], self.robots[i].radius,
                                            facecolor=(*robot_colors[i], 0.9),  # 添加透明度
                                            edgecolor='black', linewidth=1.0, zorder=2)
                        ax.add_patch(robot_circle)
                        robot_markers.append(robot_circle)
                    # Plot robots (五角星, 在上层)
                    # robot_markers = []
                    # for i in range(self.robot_num):
                    #     # 调整五角星大小与人类圆形一致
                    #     # 星星的点面积(s参数)与圆的面积关系: s = π * r² * 调整系数
                    #     star_size = self.robots[i].radius  # 使用与人类相同的半径
                    #     # 计算等效面积 (星形需要更大的s值才能与圆形视觉上相当)
                    #     area_factor = 20000  # 调整因子，使星形看起来与圆形大小相当
                    #     robot_star = plt.scatter(
                    #         robot_positions[i][k][0], robot_positions[i][k][1],
                    #         marker='*', s=star_size**2 * area_factor,  # 调整大小
                    #         color=(*robot_colors[i], 1.0),  # 添加透明度
                    #         edgecolor='black', linewidth=1.0, zorder=3
                    #     )
                    #     robot_markers.append(robot_star)
                    ...
                    agents = humans + [plt.Circle(robot_positions[i][k], self.robots[i].radius) for i in range(self.robot_num)]
                    times = [ax.text(agent.center[0] + x_offset, agent.center[1] + y_offset,
                                '{:.1f}'.format(global_time),
                                color='black', fontsize=8, zorder=4)
                            for agent in agents]

            # Plot running path lines for each robot and human
            # 先画路径线，再画点，确保点在上面
            ...
            for i in range(len(self.humans)):
                ...
                ax.plot(human_path_x, human_path_y, color=human_dark_colors[i], linewidth=1.5, alpha=0.6, zorder=1)
            
            # 画机器人路径线
            for i in range(self.robot_num):
                robot_path_x = [pos[0] for pos in robot_positions[i]]
                robot_path_y = [pos[1] for pos in robot_positions[i]]
                ax.plot(robot_path_x, robot_path_y, color=robot_dark_colors[i], linewidth=2, alpha=0.7, zorder=1)
            ...
            # Add improved legend with star markers for robots and circle markers for humans
            legend_elements = []
            ...
            for i in range(self.robot_num):
                legend_elements.append(
                    plt.Line2D([0], [0], marker='o', color='w', 
                              label=f'Robot {i+1}',
                              markerfacecolor=robot_medium_colors[i],  # 使用中间颜色
                              markersize=10,  # 放大图例中的标记
            ...
            for i in range(len(self.humans)):
                legend_elements.append(
                    plt.Line2D([0], [0], marker='o', color='w', 
                              label=f'Human {i+1}',
                              markerfacecolor=None,  # 使用中间颜色
                              markersize=8,
                              markeredgecolor=human_medium_colors[i], 
                              markeredgewidth=4)
                )
