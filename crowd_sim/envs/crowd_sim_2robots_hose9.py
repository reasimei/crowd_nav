...
class CrowdSim(gym.Env):
    ...
    def step1(self, actions, update=True):
        ...
        if args.hose:
            ...
            # Check for hose collisions with humans
            for human in self.humans:
                ...
                for i in range(len(distance_between_robots_pair)):
                    distance_to_hose = point_to_hose_curve(human_pos, 
                                                        self.robots[i*2].get_position(), 
                                                        self.robots[i*2+1].get_position(), 
                                                        self.hose_length)
                    if distance_to_hose < (human.radius + self.hose_thickness):
                        #total_reward += self.hose_collision_penalty
                        collision = True
                        break
        ...
        for i in range(self.robot_num):
            ...
            # 获取机器人的初始位置和目标位置，用于确定移动方向
            robot_start = np.array([self.robots[i].px, self.robots[i].py])
            robot_goal = np.array(self.robots[i].get_goal_position())
            
            # 计算到目标点的距离
            goal_dist = norm(end_position[i] - robot_goal)
            
            # 根据不同的移动方向调整到达目标的判断阈值
            direction_vector = robot_goal - robot_start
            if abs(direction_vector[0]) > abs(direction_vector[1]):
                # 主要水平移动（左右）
                if direction_vector[0] > 0:  # 左到右
                    # 左到右移动可能过头，使用更严格的条件
                    threshold = self.robots[i].radius * 1.5
                else:  # 右到左
                    # 右到左移动可能不足，使用宽松一点的条件
                    threshold = self.robots[i].radius * 2.2
            else:
                # 主要垂直移动（上下）
                if direction_vector[1] > 0:  # 下到上
                    # 下到上移动可能不足，使用宽松一点的条件
                    threshold = self.robots[i].radius * 2.2
                else:  # 上到下
                    # 上到下移动可能过头，使用更严格的条件
            ...
            # 更新到达目标的判断条件
            reaching_goal[i] = goal_dist < threshold
        ...
        if self.global_time >= self.time_limit - 1:
            ...
            if any(not human.reached_destination() for human in self.humans):
                collision_penalty *= 2
        ...
        # Check if reaching goal
            ...
            reward = 0
            # # Calculate hose penalties first
            # for i in range(len(distance_between_robots_pair)):
            #     if distance_between_robots_pair[i] > self.hose_length:
            #         reward += self.hose_penalty * (distance_between_robots_pair[i] - self.hose_length)
            ...
            for i, robot in enumerate(self.robots):
                ...
                if goal_distance < 0.8:
                    ...
                    ideal_speed = max(0.2, goal_distance * 0.8)  # 距离越近，理想速度越小
                    
                    # 惩罚过高的速度
                    if speed > ideal_speed:
                        speed_penalty = 0.15 * (speed - ideal_speed)
                ...
                if self.prev_goal_distances[i] is not None:
                    ...
                    # 奖励朝目标的进展
                    if goal_progress > 0:
                        progress_reward = 0.1 * goal_progress
                        reward += progress_reward
                        detailed_rewards[f'robot{i}_progress'] = progress_reward
                    # 轻微惩罚远离目标
                    elif goal_progress < -0.05:  # 只在明显远离时惩罚
                        progress_penalty = 0.05 * abs(goal_progress)
                        reward -= progress_penalty
                        detailed_rewards[f'robot{i}_regress'] = -progress_penalty
                
                # 更新到目标的距离记录
                self.prev_goal_distances[i] = goal_distance
                
                # 记录机器人状态和到达目标情况
                logging.debug(f"Robot {i}: dist={goal_distance:.2f}, " +
                             f"vel=[{current_velocity[0]:.2f}, {current_velocity[1]:.2f}], " +
                             f"reaching_goal={reaching_goal[i]}")
            
            # 区分分层强化学习和原来
            if robot is not None and (robot.policy.name == 'H-SARL' or robot.policy.name == 'H-LLM-SARL'):
                if not all_humans_reached:
                    # Phase 1: Human Avoidance
                    if active_humans_nearby:
                        # Reward for successful avoidance
                        if min_human_dist > self.discomfort_dist:
                            reward += 0.1 * min_human_dist
                        else:
                            reward -= 0.2 * (self.discomfort_dist - min_human_dist)
                        
                        # Penalty for unnecessary movement when no humans nearby
                        for robot in self.robots:
                            if np.linalg.norm([robot.vx, robot.vy]) > 0.1:
                                reward -= 0.05
                    else:
                        # Reward for staying still when no humans nearby
                        all_robots_still = True
                        for robot in self.robots:
                            if np.linalg.norm([robot.vx, robot.vy]) > 0.1:
                                all_robots_still = False
                                break
                        if all_robots_still:
                            reward += 0.1
                else:
                    # Phase 2: Goal Navigation
                    for i, robot in enumerate(self.robots):
                        # Calculate progress toward goal
                        prev_dist_to_goal = np.linalg.norm([robot.px - robot.gx, robot.py - robot.gy])
                        next_pos = robot.compute_position(actions[i], self.time_step)
                        new_dist_to_goal = np.linalg.norm([next_pos[0] - robot.gx, next_pos[1] - robot.gy])
                        
                        # Reward progress only if path is clear
                        path_clear = True
                        for human in self.humans:
                            if point_to_segment_dist(robot.px, robot.py, 
                                                next_pos[0], next_pos[1],
                                                human.px, human.py) < human.radius + robot.radius + 0.3:
                                path_clear = False
                                break
                        
                        if path_clear:
                            progress_reward = (prev_dist_to_goal - new_dist_to_goal) * 0.1
                            reward += max(0, progress_reward)
                        else:
                            reward -= 0.1  # Small penalty for attempting to move through occupied space

            # Add hose constraint rewards/penalties
            if args.hose:
                for i in range(0, self.robot_num, 2):
                    if i + 1 < self.robot_num:
                        ...
                        # 更温和的软管约束
                        #optimal_dist = self.hose_length * 0.5
                        dist_diff = dist - self.hose_length
                        if dist_diff < -1.0:
                            reward += dist_diff * 3
                        elif dist_diff > 0:
                            reward -= dist_diff * 0.5  # 降低惩罚强度
                        else:
