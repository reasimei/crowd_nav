    ...
    def step1(self, actions, update=True):
        ...
        for i in range(len(distance_between_robots_pair)):
            if distance_between_robots_pair[i] > self.hose_length:
                total_reward += self.hose_penalty * (distance_between_robots_pair[i] - self.hose_length)
        ...
        if self.global_time >= self.time_limit - 1:
            ...
            # Check zone for collision penalty multiplier
            collision_multiplier = 3.0 if any(self.is_in_human_zone(robot.px, robot.py) 
                                            for robot in self.robots) else 1.0
            total_reward += self.collision_penalty * collision_multiplier
            done = True
            info = Collision()
            ...
            # Apply zone-based multiplier for discomfort
            discomfort_multiplier = 3.0 if any(self.is_in_human_zone(robot.px, robot.py) 
                                            for robot in self.robots) else 1.0
            total_reward += (dmin - self.discomfort_dist) * self.discomfort_penalty_factor * \
                        discomfort_multiplier * self.time_step
            done = False
            info = Danger(dmin)
        else:
            # Initialize progress_reward
            progress_reward = 0
            
            # Add small positive reward for progress in robot zone
            for robot in self.robots:
                if not self.is_in_human_zone(robot.px, robot.py):
                    goal_direction = np.array([robot.gx - robot.px, robot.gy - robot.py])
                    direction_norm = np.linalg.norm(goal_direction)
                    
                    if direction_norm > 0:
                        # Normalize goal direction
                        goal_direction = goal_direction / direction_norm
                        current_velocity = np.array([robot.vx, robot.vy])
                        
                        # Calculate progress reward with normalized direction
                        current_progress_reward = np.dot(current_velocity, goal_direction) * 0.3
                        progress_reward = max(0, current_progress_reward)
                        total_reward += progress_reward
                        
                        # print(f"Debug - Robot progress:")
                        # print(f"  Position: ({robot.px:.2f}, {robot.py:.2f})")
                        # print(f"  Goal: ({robot.gx:.2f}, {robot.gy:.2f})")
                        # print(f"  Velocity: ({robot.vx:.2f}, {robot.vy:.2f})")
                        # print(f"  Progress reward: {current_progress_reward:.4f}")
            
            # # Add small negative reward for progress in human zone
            # for robot in self.robots:
            #     if self.is_in_human_zone(robot.px, robot.py):
            #         goal_direction = np.array([robot.gx - robot.px, robot.gy - robot.py])
            #         direction_norm = np.linalg.norm(goal_direction)
            ...
            #             # Calculate progress reward with normalized direction
            #             current_progress_reward = np.dot(current_velocity, goal_direction) * 0.3
            #             progress_reward = min(0, current_progress_reward)
            #             total_reward += progress_reward
                        
            # print(f"Debug - Total progress reward: {total_reward:.4f}")
            done = False
            info = Nothing()
